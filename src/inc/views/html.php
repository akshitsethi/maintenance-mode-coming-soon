			...
			<div class="as-content">
				...
				if ( ! empty( $options['mailchimp_api'] ) && ! empty( $options['mailchimp_list'] ) ) {
					...
					if ( isset( $_POST[ Config::PREFIX . 'email' ] ) ) {
						...
						if ( empty( $email ) ) {
							$code     = 'error';
							$response = $this->get_option( esc_html( stripslashes( $options['message_noemail'] ) ), esc_html__( 'Please provide your email address.', 'maintenance-mode-coming-soon' ) );
						} else {
					...
					$arrange['form'] .= '<form role="form" method="post">
							<input type="text" name="' . Config::PREFIX . 'email" placeholder="' . esc_attr( $this->get_option( sanitize_text_field( $options['input_text'] ), esc_html__( 'Enter your email address..', 'maintenance-mode-coming-soon' ) ) ) . '">
							<input type="submit" name="' . Config::PREFIX . 'submit" value="' . esc_attr( $this->get_option( sanitize_text_field( $options['button_text'] ), esc_html__( 'Subscribe', 'maintenance-mode-coming-soon' ) ) ) . '">
						</form>';
